...
const getCountryData = function (countryName) {
    ...
    request.open('GET', url + 'name/' + countryName);
    request.send();
...
const getJSON = function (url, errMsg = 'Something is no yes') {
    return fetch(url).then(response => {
        if (!response.ok) throw new Error(`${errMsg}: ${response.status}`)
        return response.json()
    })
}
const getCountryDataFetch = function (countryName) {
    getJSON(url + 'name/' + countryName,).then(data => {
        renderCountry(data[0])
        const neighbour = data[0].borders?.[0];
        if (!neighbour) throw new Error('No neighbour found')
        return getJSON(url + 'alpha/' + neighbour, `Country not found: ${countryName}`)
...
element with the 'images' class, and resolve the promise.
